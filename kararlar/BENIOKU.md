# Karar kayıtları

Bu klasörde mimari kararlar tutulur: **neyin neden öyle olduğu**. Kod ne
yaptığını zaten anlatır; buradaki kayıtlar hangi seçeneğin neden elendiğini
ve hangi tuzağa düşülmemesi gerektiğini anlatır.

Bir kararı değiştiriyorsanız eski kaydı **silmeyin**. Durumunu
`Değiştirildi (bkz. NNNN)` yapın ve yeni kayıt yazın; kararın neden
değiştiği de tarihin parçasıdır.

Yeni kayıt `NNNN-kisa-baslik.md` olarak numaralanır ve şu başlıkları taşır:
Durum, Bağlam, Karar, Gerekçe, Sonuçlar.

| No | Karar | Durum |
|---|---|---|
| [0001](0001-cevrimdisi-ve-yerel-veri.md) | Çevrimdışı çalışma, veri kurumda kalır | Kabul |
| [0002](0002-kural-motoru-veritabani-gormez.md) | Kural motoru veritabanı görmez | Kabul |
| [0003](0003-belgeler-koddan-uretilir.md) | Belgeler koddan üretilir, şablondan değil | Kabul |
| [0004](0004-sema-goclerle-ilerler.md) | Şema göçlerle ilerler, veri ayrı klasörde | Kabul |
| [0005](0005-ilanlarda-kisi-adi-yok.md) | İlan çıktılarında kişi adı geçmez | Kabul |
| [0006](0006-basvuru-kapisi-pencereye-bagli.md) | Başvuru kapısı oturuma değil pencereye bağlı | Kabul |
| [0007](0007-surum-tek-kaynakta.md) | Sürüm tek kaynakta tutulur | Kabul |
| [0008](0008-kurulum-kullanici-basina.md) | Kurulum kullanıcı başınadır | Kabul |
| [0009](0009-pardus-paketi-pyinstaller-ile.md) | Pardus paketi bütünleşiktir | Kabul |
